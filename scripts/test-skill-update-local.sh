...
echo "$upd" | grep -q "perms=\[" || { echo "[skill-update] FAIL reason=permissions-hint-missing"; echo "$upd"; exit 1; }
...
echo "$after" | grep -q "perms=\[" || { echo "[skill-update] FAIL reason=list-permissions-missing"; echo "$after"; exit 1; }
